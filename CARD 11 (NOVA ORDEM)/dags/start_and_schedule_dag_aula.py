...
default_args = {
    'start_date': datetime(2024, 11, 28, 1),
    'owner': 'Airflow'
}

# o dummy operator é um operador que não faz nada, só serve para marcar o início e o fim de um fluxo

with DAG(dag_id='start_and_schedule_dag', schedule_interval="0 * * * *", default_args=default_args) as dag:

    # Task 1: dummy_task_1 é um operador dummy que não faz nada
    ...
    # código comentado para depuração (logs informativos sobre execuções da DAG)
    # útil apenas durante o desenvolvimento e pode ser descomentado para testes

    # o tutorial diz para descomentar isso quando fizer os testes
